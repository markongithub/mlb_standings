...
class SeasonParameters(object):
    ...
    def use_half_seasons(self, team):
        ...
        return league in ['Eastern League', 'Southern League', 'Texas League', 'Midwest League', 'Northwest League', 'South Atlantic League', 'California League', 'Florida State League']
...
def is_division_contender_with_rivalries(played, unplayed, season_params: SeasonParameters, date, team: str):
    ...
    threats = divisional_threats(compute_standings(played.loc[(played['completion_date'] <= date)]), season_params, team)
...
#    return merge1.loc[merge1['max_wins'] >= merge1['W_y']][['W_x', 'L', 'div', 'lg', 'max_wins', 'division_leader']].rename(columns={'W_x': 'W'})
    return merge1[['W_x', 'L', 'div', 'lg', 'max_wins', 'division_leader']].rename(columns={'W_x': 'W'})
...
def games_between_wildcard_rivals_after_date(played, unplayed, season_params: SeasonParameters, date, team):
    ...
    standings = wildcard_standings(compute_standings(played.loc[(played['completion_date'] <= date)]), season_params)
...
def count_teams(played, unplayed, divisions):
    schedule_teams = sorted(pd.unique(pd.concat([played, unplayed])[['home', 'visitor']].values.ravel('K')))
...
def elo_to_played_and_unplayed(elo_immutable):
    ...
    elo = elo.rename(columns={'team1': 'home', 'team2': 'visitor', 'date': 'completion_date', 'score1': 'home_score', 'score2': 'visitor_score'})
...
def show_dumb_elimination_output4(played, unplayed, season_params):
    ...
    print(f'The top {season_params.winners_per_division} teams from each division go to the postseason.')
    ...
    while (
        ...
        for supposed_contender in div_contenders.copy():
            ...
            # waste CPU time on you.
            if tomorrows_div_contenders and supposed_contender not in tomorrows_div_contenders:
                if not is_division_contender_with_rivalries(played, unplayed, season_params, current_date, supposed_contender):
                    print(f'It looked like the {display_name(season_params.divisions, supposed_contender)} were in contention after {date_str} but the remaining intra-division games ruled them out.')
                    div_contenders.remove(supposed_contender)
        new_contenders = set()
        if tomorrows_div_contenders:
            new_contenders = div_contenders.difference(tomorrows_div_contenders)
        ...
        for eliminated_team in new_contenders:
            ...
            print(f'The {display_name(season_params.divisions, eliminated_team)} were eliminated from the {elim_div} title on {datetime_to_retro(tomorrow)} with {games_to_go_at_elimination} games left to play.')
            new_pair = (datetime_to_retro(tomorrow), games_to_go_at_elimination)
            elim_franchise = season_params.divisions.loc[eliminated_team]['franchise_id'] 
            if eliminations.get(elim_franchise):
                eliminations[elim_franchise]['division'] = new_pair
            else:
                eliminations[elim_franchise] = {'division': new_pair}
        ...
        for supposed_contender in wildcard_contenders.copy():
            ...
            if tomorrows_wildcard_contenders and supposed_contender not in tomorrows_wildcard_contenders:
                # print(f'let\'s see if {supposed_contender} is really still in wildcard contention')
                if not is_wild_card_contender_with_rivalries(played, unplayed, season_params, current_date, supposed_contender):
                    print(f'It looked like the {display_name(season_params.divisions, supposed_contender)} were in wildcard contention after {date_str} but the remaining intra-contender games ruled them out.')
                    wildcard_contenders.remove(supposed_contender)
        new_contenders = set()
        if tomorrows_wildcard_contenders:
            new_contenders = wildcard_contenders.difference(tomorrows_wildcard_contenders)
        for eliminated_team in new_contenders:
            ...
            print(f'The {display_name(season_params.divisions, eliminated_team)} were eliminated from {elim_lg} wildcard contention on {datetime_to_retro(tomorrow)} with {games_to_go_at_elimination} games left to play.')
            new_pair = (datetime_to_retro(tomorrow), games_to_go_at_elimination)
            elim_franchise = season_params.divisions.loc[eliminated_team]['franchise_id'] 
            if eliminations.get(elim_franchise):
                eliminations[elim_franchise]['wildcard'] = new_pair
...
def get_division_rivals(standings, divisions, team):
    ...
    df = standings.merge(divisions, left_index=True, right_index=True).loc[divisions['div'] == my_division]
...
def get_wild_card_rivals(standings_immutable, season_params, team):
    ...
    standings = standings.drop(standings.nlargest(season_params.wildcard_count - 1, columns=['W']).index)
...
def get_elimination_number(standings, head_to_head, remaining, season_length, team1, team2):
    ...
    if max_h2h_margin == 0:
        if remaining_h2h == 0:
            print(f'Um there is a guaranteed head-to-head tie between {team1} and {team2} and I am not sure how to handle that.')
        else:
            print(f'Um there might be a head-to-head tie between {team1} and {team2} and I am not sure how to handle that.')
    okay_to_tie = max_h2h_margin > 0
    if 'OAK' in [team1, team2] and 'HOU' in [team1, team2]:
        print(f'for HOU and OAK, h2h_wins {h2h_wins} h2h_losses {h2h_losses} remaining_h2h {remaining_h2h}')
...
def all_elimination_numbers(standings, head_to_head, remaining, remaining_indexed, season_length, team, rivals):
    eliminations = {rival: get_elimination_number(standings, head_to_head, remaining_indexed, 162, team, rival) for rival in rivals}
    set_eliminations = []
    for subset in powerset(rivals):
        if len(subset) < 1:
            continue
        subset_matchup_count = remaining.loc[(remaining['alpha1'].isin(subset)) & (remaining['alpha2'].isin(subset))]['size'].sum()
        if len(subset) > 1 and (not subset_matchup_count):
            continue
        total_elimination = sum([eliminations[rival] for rival in subset])
        new_tuple = (total_elimination - subset_matchup_count, team, subset, total_elimination, subset_matchup_count)
        if new_tuple[0] < 20:
            set_eliminations.append(new_tuple)
    return set_eliminations

def get_division_contenders3(played_orig, unplayed, season_params: SeasonParameters, date=None):
    if not date:
    ...
    remaining = all_matchups_after_date(played_orig, unplayed, date).groupby(['alpha1', 'alpha2'], as_index=False).size()
    ...
    for team in standings.index:
        ...
        set_eliminations += all_elimination_numbers(standings, head_to_head, remaining, remaining_indexed, 162, team, rivals)
...
def run_statsapi():
    ...
    PLAYED, UNPLAYED = statsapi_schedule_to_played_unplayed('./data/schedule_2022_14.json')
...
def run_statsapi_just_elimination_numbers():
    teams_dict = json.load(open('./data/teams.json'))
    PLAYED, UNPLAYED = statsapi_schedule_to_played_unplayed('./data/schedule_2022_1.json')
    SP2022 = SeasonParameters(2022, statsapi_played=PLAYED, statsapi_unplayed=UNPLAYED, statsapi_teams=teams_dict)
